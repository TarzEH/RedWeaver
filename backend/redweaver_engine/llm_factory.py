...
@runtime_checkable
class KeysProvider(Protocol):
    """Supplies the LLM API-key / provider-config dict.

    Decouples the engine from any persistence layer: the Django side passes
    an adapter backed by ApiKeyVault, tests pass a plain dict wrapper. Anything
    with ``get_all() -> dict`` works.
    """
...
class LLMFactory:
    ...
    def __init__(self, api_keys_repository: KeysProvider) -> None:
