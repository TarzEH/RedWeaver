...
def build_offsec_crew(
    ...
    description = (
        ...
        "   - **KB source**: the exact KB filename(s) from the RESEARCH CONTEXT whose "
        "technique/commands you used (REQUIRED whenever a KB technique block exists "
        "for this finding — quote the relevant command from it)\n"
        "   - **References**: CVE/exploit URL(s)\n"
        "## Privilege Escalation & Post-Exploitation — base this on the shared KB "
        "methodology blocks and cite their KB filenames (e.g. linux-privesc-methodology.md).\n"
        "## Cleanup & Notes\n\n"
